...
Die erste CRD ist eine Ausnahme davon. Das Gateway konfiguriert nicht die anderen Envoy-Proxies. Trotzdem ist auch das Gateway ein eigener Envoy-Proxy.
...
Auch lässt sich damit konfigurieren, wie der Traffic zwischen zwei Services prozentual aufgeteilt werden soll, beispielsweise für ein Canary Deployment. Zudem sind Failover Einstellungen möglich und Weiteres.
...
Auch über die CRD "ProxyConfig" können die Sidecars genauer konfiguriert werden, jedoch in anderer Hinsicht. Über ProxyConfig können die Konfiguration für das vom Proxy verwendete Image und die für den Proxy verwendeten CPU-Cores konfiguriert werden. Dies kann sowohl für einen kompletten Namespace als auch über Label für einen bestimmten Workload konfiguriert werden. Der Einsatz dieser CRD ist vermutlich sehr selten und speziell.
